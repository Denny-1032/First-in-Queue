...
(function () {
  ...
  var unread = 0;
  var listeners = {};
  var root, panel, iframe, launcher, badge;
  ...
  function build() {
    ...
    iframe.src =
      baseUrl + '/widget/chat?key=' + encodeURIComponent(key) + (isMobile ? '&mobile=1' : '');
  ...
  function legacyVoiceWidget(script, baseUrl) {
    ...
    container.innerHTML =
      ...
      (c.showBranding
        ? '<div style="position:absolute;' + (c.position.indexOf('bottom') > -1 ? 'bottom:-20px' : 'top:-20px') + ';' + (c.position.indexOf('right') > -1 ? 'right:0' : 'left:0') + ';font-size:10px;color:#999">Powered by First in Queue</div>'
        : '');
