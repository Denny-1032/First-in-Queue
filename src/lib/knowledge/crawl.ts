...
// =============================================
// Website crawler core (extracted from /api/knowledge/crawl).
// ---------------------------------------------
// Pure of HTTP-framework concerns so it can be reused by both the dashboard
// crawl route and the onboarding wizard (§7 step 1). Behaviour is unchanged
// from the original inline implementation — this is a move, not a rewrite.
// =============================================
...
/** Thrown for a bad input URL (→ 400) or a site that yielded nothing (→ 422). */
...
/**
 * Crawl up to {@link MAX_PAGES} business-relevant pages from `url` and convert
 * them into structured knowledge entries. Blocking and slow (tens of seconds);
 * callers that must stay responsive should run it detached.
 */
export async function crawlSite(url: string): Promise<CrawlResult> {
  if (!url) throw new CrawlError("url is required", 400);
  ...
  try {
    ...
    throw new CrawlError("Invalid URL. Please provide a valid http or https URL.", 400);
  }
  ...
  if (crawledPages.length === 0) {
    throw new CrawlError(
      "Could not crawl any pages from this URL. The site may be blocking automated requests.",
      422
    );
  }
